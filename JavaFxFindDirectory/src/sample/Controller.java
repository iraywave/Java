...
public class Controller {
    ...
    public void clickBtn(){
        result_fld.clear();
        File file = new File(path_fld.getText()); // path_fld - текстовое поле, где пользователь задает путь
        if (file.isDirectory()){ // если файл является директорией
            ...
            if (arrayList.size() > 0){
                ...
                Alert alert = new Alert(Alert.AlertType.ERROR, "Попробуйте позже");
                alert.showAndWait();
            }
        }
        else {
